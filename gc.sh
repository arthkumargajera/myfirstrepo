...
FILE=accounts/1.json
if test -f "$FILE"; then
    echo "$FILE Folder Exist."
else
...
if [ $MD == "A" ]; then
  ...
  read DST
  gclone copy GC:{$SRC} GC:{$DST} --config=rc.conf --transfers 50 -v --stats-one-line --stats=15s --ignore-existing --drive-server-side-across-configs --drive-chunk-size 128M --drive-acknowledge-abuse --drive-keep-revision-forever
elif [ $MD == "B" ]; then
  ...
  echo -en "Provide DST Folder ID\n"
  read DST
  gclone move GC:{$SRC} GC:{$DST} --config=rc.conf --transfers 50 -v --stats-one-line --stats=15s --ignore-existing --drive-server-side-across-configs --drive-chunk-size 128M --drive-acknowledge-abuse --drive-keep-revision-forever
elif [ $MD == "C" ]; then
  ...
  echo -en "Provide DST Folder ID\n"
  read DST
  gclone sync GC:{$SRC} GC:{$DST} --config=rc.conf --transfers 50 -v --stats-one-line --stats=15s --ignore-existing --drive-server-side-across-configs --drive-chunk-size 128M --drive-acknowledge-abuse --drive-keep-revision-forever
elif [ $MD == "D" ]; then
